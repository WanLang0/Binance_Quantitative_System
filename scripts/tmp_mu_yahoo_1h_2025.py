# -*- coding: utf-8 -*-
"""美光真实股票 MU（Yahoo Finance 渠道）2025全年+2026至今 1h 回测扫描

背景：币安 MU/MUB/MUUB 为美光科技(Micron)代币，币安仅有2026年上市以来的短K线。
经与用户确认：改用 Yahoo Finance(yfinance) 拉取美光真实股票 MU 的 1h K线，
覆盖 2025-01-01 ~ 今，扫描策略找出收益最高者，入库并标注渠道名称。

渠道：Yahoo Finance（yfinance）美股小时线（1h）
口径：美股个股现货仅做多为主，双向仅作参考对比
入库：收益最高(仅做多口径)记录，note/source 标注渠道
"""
...
def fetch_1h(ticker, start='2025-01-01', tries=5):
    for i in range(tries):
        try:
            ...
            # 展平 MultiIndex 列（yfinance 会加 Ticker 层）
            if isinstance(df.columns, pd.MultiIndex):
                df.columns = df.columns.get_level_values(0)
            df = df[['Open', 'High', 'Low', 'Close', 'Volume']].copy()
            df.columns = ['open', 'high', 'low', 'close', 'volume']
            df.index.name = 'timestamp'
            # 剔除 NaN 行（部分日期收盘栏为 NaN）
...
print(f"\n>>> {TICKER}（{CHANNEL}）: {len(df)}根/{days}天 ({per})  买入持有 {buy_hold:+.1f}%")
...
print(f"\n入库: {top['strat']} {top['mode']} {top['tpsl']} {top['ret']:+.1f}% → {msg}")
if ok:
...
print('saved → scripts/results/mu_yahoo_1h_2025.json')
