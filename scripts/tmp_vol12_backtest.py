# -*- coding: utf-8 -*-
"""量能阈值对照回测：1.5x（基准）vs 1.2x
三种MACD参数(12/26/9, 12/16/5, 12/16/7) × 杠杆(1/2/4)，1h 周期。
口径与 tmp_macd_3param_monthly.py 完全一致：30币均分30份独立复利, 0.05%/边,
C判定(止损收盘确认/止盈盘中触发), ATR14止1.5/盈2截断[1%,8%]。
输出: 总收益/MDD/单币MDD/笔数/胜率/逐月收益 + 与1.5x基准的对照。
"""
...
def simulate_lev(df, signals, initial, atr, k_sl, k_tp, lev, t0):
    ...
    for i in range(len(df)):
        ...
        if side != 0 and entry > 0:
            if side > 0:
                ...
                hit_tp_i = hi[i] >= tp_lvl
                hit_sl_c = price <= sl_lvl
            else:
                tp_lvl, sl_lvl = entry * (1 - cur_tp), entry * (1 + cur_sl)
                hit_tp_i = lo[i] <= tp_lvl
                hit_sl_c = price >= sl_lvl
...
def run(data, f, s, g, lev, vol_mult):
...
def main():
    ...
    results = {}
    for vol_mult in (1.5, 1.2):
        tag = f'vol{str(vol_mult).replace(".", "_")}x'
        results[tag] = {}
        for name, (f, s, g) in VARIANTS.items():
            out = {}
            for lev in (1, 2, 4):
                out[f'lev{lev}x'] = run(data, f, s, g, lev, vol_mult)
            results[tag][name] = out
        print(f'== 量能阈值 {vol_mult}x 完成 ==', flush=True)
    p = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', 'macd_vol_threshold_1h.json')
    ...
    for name in VARIANTS:
        print(f'\n===== {name} (1h) =====', flush=True)
        for lev in (1, 2, 4):
            b = results['vol1_5x'][name][f'lev{lev}x']
            n = results['vol1_2x'][name][f'lev{lev}x']
            print(f'  [{lev}x] 1.5x基准: 总{b["ret"]:>+9.1f}% MDD{b["mdd"]:>7.1f}% 单币MDD均{b["coin_mdd_avg"]:>6.1f}% '
                  f'笔{b["trades"]:>5} 胜{b["winrate"]}%', flush=True)
            print(f'        1.2x新版: 总{n["ret"]:>+9.1f}% MDD{n["mdd"]:>7.1f}% 单币MDD均{n["coin_mdd_avg"]:>6.1f}% '
                  f'笔{n["trades"]:>5} 胜{n["winrate"]}%', flush=True)
            print(f'        年度对比 1.5x {b["yearly"]} -> 1.2x {n["yearly"]}', flush=True)
    print(f'\n逐月明细 → {p}')
