# -*- coding: utf-8 -*-
"""美股七姐妹（Magnificent 7）Yahoo Finance 1h 回测：2025全年 + 2026至今

与 QQQ/AAPL/NVDA 同渠道：yfinance 拉美股 1h K线，覆盖 2025-01-01 ~ 今。
七姐妹 = AAPL 苹果 / MSFT 微软 / GOOGL 谷歌(Alphabet) / AMZN 亚马逊 / NVDA 英伟达 / META Meta / TSLA 特斯拉。
每标的独立扫描策略，仅做多口径收益最高者入库，标注渠道。
"""
...
TICKERS = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'NVDA', 'META', 'TSLA']  # 美股七姐妹
NAMES = {'AAPL': '苹果', 'MSFT': '微软', 'GOOGL': '谷歌(Alphabet)',
...
for TICKER in TICKERS:
    ...
    print(f"\n>>> {TICKER}（{NAME}）: {len(df)}根/{days}天 ({per})  买入持有 {buy_hold:+.1f}%")
    ...
    for strat, ip in COMBOS.items():
        ...
        for mode in MODES:
            for tp, sl in TPSL:
                ...
                if r is None:
                    continue
                out.append({'ticker': TICKER, 'name': NAME, 'strat': strat,
    ...
    note = (f"{CHANNEL}，{NAME}美股真实股票{TICKER}，{days}天{top['n']}笔；"
...
with open('scripts/results/m7_yahoo_1h_2025.json', 'w', encoding='utf-8') as f:
    json.dump(all_results, f, ensure_ascii=False, indent=1, default=float)
print('\nsaved → scripts/results/m7_yahoo_1h_2025.json')
