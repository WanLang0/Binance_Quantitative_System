# -*- coding: utf-8 -*-
"""美股真实股票扫描（Yahoo Finance 渠道）：QQQ/AAPL/NVDA 2025全年+2026至今 1h 回测

与美光 MU 同渠道：yfinance 拉美股 1h K线，覆盖 2025-01-01 ~ 今。
每个标的独立扫描策略，仅做多口径收益最高者入库，标注渠道。
备注：QQQ=纳指100 ETF、AAPL=苹果、NVDA=英伟达（美股真实股票，非币安代币）。
"""
...
TICKERS = ['QQQ', 'AAPL', 'NVDA']  # QQQ=纳指100 ETF, AAPL=苹果, NVDA=英伟达
...
for TICKER in TICKERS:
    NAME = {'QQQ': '纳指100 ETF', 'AAPL': '苹果', 'NVDA': '英伟达'}[TICKER]
    df = fetch_1h(TICKER)
    if df is None:
...
with open('scripts/results/us_stocks_yahoo_1h_2025.json', 'w', encoding='utf-8') as f:
    json.dump(all_results, f, ensure_ascii=False, indent=1, default=float)
print('\nsaved → scripts/results/us_stocks_yahoo_1h_2025.json')
