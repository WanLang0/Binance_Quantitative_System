# -*- coding: utf-8 -*-
"""2021 年初市值前30可测池(26币) × 三种止盈止损口径对照 × 2021-01~2024-01 回测
...
def main():
    data = load_data()
    print(f'2021年初市值前30可测池({len(data)}币) · 三口径对照 · 1h · 窗口2021-01-01~2024-01-01\n', flush=True)
    out = {}
    for name, (f, s, g) in VARIANTS.items():
        ...
        for sname, (k_sl, k_tp, tp_mode) in SCHEMES.items():
            ...
            print('', flush=True)
    p = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', 'top30_2021_tpsl_compare_1h.json')
