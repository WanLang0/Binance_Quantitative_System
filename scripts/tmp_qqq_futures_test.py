# -*- coding: utf-8 -*-
"""QQQ 合约测试入库：7策略×2模式×2止盈止损=28组（合约口径，同新股扫描），
...
with open('scripts/results/qqq_futures_test.json', 'w', encoding='utf-8') as f:
    json.dump(results, f, ensure_ascii=False, indent=1, default=float)

# ---- Top1 入库（全链路：ret/夏普/胜率/流水/日期）----
SRC = 'QQQ合约测试（2026-08）'
for coin, out in results.items():
    ...
    rec = {'symbol': top['sym'], 'timeframe': '4h', 'strategy': top['strat'],
           ...
           'note': f"QQQ纳指100 ETF代币，28组合约扫描冠军；美股代币波动小于加密币，参数仅供参考",
...
print('\nsaved → scripts/results/qqq_futures_test.json')
